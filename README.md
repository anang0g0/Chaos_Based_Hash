...
https://www.ai-gakkai.or.jp/jsai2014/webprogram/2014/pdf/64.pdf

へえー、英語なんか読めなくても安全な秘密鍵暗号って作れるんですね。
私も頑張ろうっと。

確かに自分のイメージ通りにはできてるけど、これってSPNのように見えて仕方がない。
あえて言えば両方がごっっちゃになってる感じ。

ブーフマンの本に書いてあることが正しければ、秘密鍵暗号というものは置換と換字だけで作れるはずだ。
私のアプローチではFeistel構造のように左右を半分に分けて、その部分だけにF関数をかけるのではなく、置き換えの場所を固定せずに動的にすべてのブロックサイズに対して置換をかけるというものだ。
...
バグをとった。
セッションカギを変化させてなかったのと、それをXORすると復元できないエラー。
